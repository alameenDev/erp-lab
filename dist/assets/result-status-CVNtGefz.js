import{M as R,b as S}from"./vue-vendor-DnM_FKQV.js";import{$ as C}from"./index-DGMgdG2Z.js";import"./utils-DOjieFrw.js";function P(){return{printWithIframe:async(r,t,i="Print",o=100,a=null)=>(a&&(await a(),await R()),new Promise((e,n)=>{setTimeout(()=>{const s=document.getElementById(r)?.innerHTML;if(!s){console.error(`Element with id "${r}" not found`),n(new Error(`Element not found: ${r}`));return}const p=document.createElement("iframe");p.style.cssText="position: absolute; width: 0px; height: 0px; border: none;",document.body.appendChild(p);const d=p.contentWindow.document;d.open(),d.write(`
          <html>
            <head>
              <title>${i}</title>
              <style>${t}</style>
        ...
        `),d.close(),p.contentWindow.focus(),p.contentWindow.print(),p.contentWindow.onafterprint=()=>{document.body.removeChild(p),e()}},o)})),printWithCustomContent:async(r,t,i="Print",o=50)=>new Promise(a=>{setTimeout(()=>{const e=document.createElement("iframe");e.style.cssText="position: absolute; width: 0px; height: 0px; border: none;",document.body.appendChild(e);const n=e.contentWindow.document;n.open(),n.write(`
          <html>
            <head>
              <title>${i}</title>
        ...
        `),n.close(),e.contentWindow.focus(),e.contentWindow.print(),e.contentWindow.onafterprint=()=>{document.body.removeChild(e),a()}},o)}),printStyles:{jobOrder:`
    ...
    `,getBarcodeCss:r=>{const t=r||{},i=t.label_width||3,o=t.label_height||1.5,a=t.name_size||9,e=t.info_size||7,n=t.number_size||6,s=t.barcode_height||40,p=t.sample_size||8,d=t.tests_size||7;return`
        @page { size: ${i}in ${o}in; margin: 0 !important; }
        * { margin: 0; padding: 0; box-sizing: border-box; }
        html, body { width: ${i}in; height: ${o}in; }
        body { font-family: Arial, sans-serif; color: #000; }
        .lbl { width: ${i}in; height: ${o}in; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 0.08in 0.15in; overflow: hidden; }
        ...
        .num { font-size: ${n}pt; font-weight: bold; line-height: 1.1; }
        .sample { font-size: ${p}pt; font-weight: bold; line-height: 1.2; text-transform: uppercase; }
        .bc { width: 95%; margin: 2px 0; }
        .bc svg { width: 100% !important; height: ${s}px !important; }
        .pname { font-size: ${a}pt; font-weight: bold; line-height: 1.3; margin: 2px 0; text-align: center; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100%; }
      ...
      `},getPrintTableCss:r=>{const t=r||{},i=(c,u=40)=>{if(typeof c!="string")return c;let l=c.trim();if(!/^#([0-9a-f]{3}|[0-9a-f]{6})$/i.test(l))return c;l.length===4&&(l="#"+[...l.slice(1)].map(b=>b+b).join(""));const w=parseInt(l.slice(1,3),16),y=parseInt(l.slice(3,5),16),z=parseInt(l.slice(5,7),16),k=.299*w+.587*y+.114*z;if(k<=u)return c;const _=u/k,x=b=>Math.round(b*_).toString(16).padStart(2,"0");return`#${x(w)}${x(y)}${x(z)}`},o=t.header_font_size||13,a=t.header_font_family||"inherit",e=i(t.header_color||"#0f172a"),n=t.header_bg_color||"#f1f5f9",s=t.header_font_weight||"bold",p=t.body_font_size||13,d=t.body_font_family||"inherit",$=t.body_color||"#1e293b",v=t.body_bg_color||"#ffffff",f=t.border_color||"#e2e8f0",h=t.cell_padding!=null?t.cell_padding:6,j=t.section_spacing!=null?t.section_spacing:12;return`
      ...
      `,getPatientHeaderCss:r=>{const t=r||{},i=t.name_size||20,o=t.info_size||13,a=t.barcode_height||35,e=t.qr_size||90,n=t.line_height||1.7;return`
        #Result .patient-header { font-size: ${o}px !important; line-height: ${n} !important; }
        #Result .patient-header .patient-name { font-size: ${i}px !important; }
        #Result .patient-header .info-table td { font-size: ${o}px !important; }
        #Result .patient-header .barcode-box svg { height: ${a}px !important; }
        #Result .patient-header .qr-col svg { width: ${e}px !important; height: ${e}px !important; }
        .patient-header { font-size: ${o}px !important; line-height: ${n} !important; }
        .patient-header .patient-name { font-size: ${i}px !important; }
        .patient-header .info-table td { font-size: ${o}px !important; }
        .patient-header .barcode-box svg { height: ${a}px !important; }
    ...
    `,getResultCss:()=>`
      ...
      `}}}const B=S("result-status",{state:()=>({resultStatus:[],dialog:!1}),actions:{async GetresultStatus(){try{const{data:g}=await C.get("/result-status");this.resultStatus=g.map(m=>({label:m.status,value:m.id}))}catch{this.resultStatus=[]}}}});export{B as a,P as u};
