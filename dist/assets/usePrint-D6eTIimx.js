import{M as m}from"./vue-vendor-BHBT87HN.js";function w(){return{printWithIframe:async(d,t,r="Print",n=100,p=null)=>(p&&(await p(),await m()),new Promise((e,o)=>{setTimeout(()=>{const l=document.getElementById(d)?.outerHTML;if(!l){console.error(`Element with id "${d}" not found`),o(new Error(`Element not found: ${d}`));return}const i=document.createElement("iframe");i.style.cssText="position: absolute; width: 0px; height: 0px; border: none;",document.body.appendChild(i);const a=i.contentWindow.document;a.open(),a.write(`
          <html>
            <head>
              <title>${r}</title>
              <style>${t}</style>
        ...
        `),a.close(),(async()=>{await Promise.all(Array.from(a.images).map(s=>s.complete?Promise.resolve():new Promise(c=>{s.onload=c,s.onerror=c}))),a.fonts?.ready&&await a.fonts.ready,i.contentWindow.onafterprint=()=>{i.remove(),e()},i.contentWindow.focus(),i.contentWindow.print()})().catch(()=>{i.remove(),e()})},n)})),printWithCustomContent:async(d,t,r="Print",n=50)=>new Promise(p=>{setTimeout(()=>{const e=document.createElement("iframe");e.style.cssText="position: absolute; width: 0px; height: 0px; border: none;",document.body.appendChild(e);const o=e.contentWindow.document;o.open(),o.write(`
          <html>
            <head>
              <title>${r}</title>
        ...
        `),o.close(),(async()=>{await Promise.all(Array.from(o.images).map(i=>i.complete?Promise.resolve():new Promise(a=>{i.onload=a,i.onerror=a}))),o.fonts?.ready&&await o.fonts.ready,e.contentWindow.onafterprint=()=>{e.remove(),p()},e.contentWindow.focus(),e.contentWindow.print()})().catch(()=>{e.remove(),p()})},n)}),printStyles:{jobOrder:`
    ...
    `,getBarcodeCss:d=>{const t=d||{},r=t.label_width||3,n=t.label_height||1.5,p=t.name_size||9,e=t.info_size||7,o=t.number_size||6,l=t.barcode_height||40,i=t.sample_size||8,a=t.tests_size||7;return`
        @page { size: ${r}in ${n}in; margin: 0 !important; }
        * { margin: 0; padding: 0; box-sizing: border-box; }
        html, body { width: ${r}in; height: ${n}in; }
        body { font-family: Arial, sans-serif; color: #000; }
        .lbl { width: ${r}in; height: ${n}in; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 0.08in 0.15in; overflow: hidden; }
        ...
        .num { font-size: ${o}pt; font-weight: bold; line-height: 1.1; }
        .sample { font-size: ${i}pt; font-weight: bold; line-height: 1.2; text-transform: uppercase; }
        ...
        .tests-row { font-size: ${a}pt; font-weight: bold; line-height: 1.2; text-align: center; width: 100%; padding: 0 0.05in; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
      `},getPrintTableCss:d=>{const t=d||{},r=t.header_font_size||13,n=t.header_font_family||"inherit",p=t.header_color||"#0f172a",e=t.header_bg_color||"#f1f5f9",o=t.header_font_weight||"bold",l=t.body_font_size||13,i=t.body_font_family||"inherit",a=t.body_color||"#1e293b",b=t.body_bg_color||"#ffffff",s=t.border_color||"#e2e8f0",c=t.cell_padding!=null?t.cell_padding:6,g=t.section_spacing!=null?t.section_spacing:12;return`
      ...
      `,getPatientHeaderCss:d=>{const t=d||{},r=t.name_size||20,n=t.info_size||13,p=t.barcode_height||35,e=t.qr_size||90,o=t.line_height||1.7;return`
        .rs-header .rs-info, .rs-header .rs-dates { font-size: ${n}px !important; line-height: ${o} !important; }
        .rs-header .rs-info > div:first-child { font-size: ${r}px !important; }
        .rs-header .rs-qr svg { width: ${e}px !important; height: ${e}px !important; }
        #Result .patient-header { font-size: ${n}px !important; line-height: ${o} !important; }
        #Result .patient-header .patient-name { font-size: ${r}px !important; }
        ...
        .patient-header { font-size: ${n}px !important; line-height: ${o} !important; }
        .patient-header .patient-name { font-size: ${r}px !important; }
