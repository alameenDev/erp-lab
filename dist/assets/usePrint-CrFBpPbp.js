import{M as j}from"./vue-vendor-BHBT87HN.js";function q(){return{printWithIframe:async(d,t,a="Print",n=100,p=null)=>(p&&(await p(),await j()),new Promise((e,i)=>{setTimeout(()=>{const l=document.getElementById(d)?.innerHTML;if(!l){console.error(`Element with id "${d}" not found`),i(new Error(`Element not found: ${d}`));return}const o=document.createElement("iframe");o.style.cssText="position: absolute; width: 0px; height: 0px; border: none;",document.body.appendChild(o);const r=o.contentWindow.document;r.open(),r.write(`
          <html>
            <head>
              <title>${a}</title>
              <style>${t}</style>
        ...
        `),r.close(),(async()=>{await Promise.all(Array.from(r.images).map(c=>c.complete?Promise.resolve():new Promise(b=>{c.onload=b,c.onerror=b}))),r.fonts?.ready&&await r.fonts.ready,o.contentWindow.onafterprint=()=>{o.remove(),e()},o.contentWindow.focus(),o.contentWindow.print()})().catch(()=>{o.remove(),e()})},n)})),printWithCustomContent:async(d,t,a="Print",n=50)=>new Promise(p=>{setTimeout(()=>{const e=document.createElement("iframe");e.style.cssText="position: absolute; width: 0px; height: 0px; border: none;",document.body.appendChild(e);const i=e.contentWindow.document;i.open(),i.write(`
          <html>
            <head>
              <title>${a}</title>
        ...
        `),i.close(),(async()=>{await Promise.all(Array.from(i.images).map(o=>o.complete?Promise.resolve():new Promise(r=>{o.onload=r,o.onerror=r}))),i.fonts?.ready&&await i.fonts.ready,e.contentWindow.onafterprint=()=>{e.remove(),p()},e.contentWindow.focus(),e.contentWindow.print()})().catch(()=>{e.remove(),p()})},n)}),printStyles:{jobOrder:`
    ...
    `,getBarcodeCss:d=>{const t=d||{},a=t.label_width||3,n=t.label_height||1.5,p=t.name_size||9,e=t.info_size||7,i=t.number_size||6,l=t.barcode_height||40,o=t.sample_size||8,r=t.tests_size||7;return`
        @page { size: ${a}in ${n}in; margin: 0 !important; }
        * { margin: 0; padding: 0; box-sizing: border-box; }
        html, body { width: ${a}in; height: ${n}in; }
        body { font-family: Arial, sans-serif; color: #000; }
        .lbl { width: ${a}in; height: ${n}in; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 0.08in 0.15in; overflow: hidden; }
        ...
        .num { font-size: ${i}pt; font-weight: bold; line-height: 1.1; }
        .sample { font-size: ${o}pt; font-weight: bold; line-height: 1.2; text-transform: uppercase; }
        ...
        .tests-row { font-size: ${r}pt; font-weight: bold; line-height: 1.2; text-align: center; width: 100%; padding: 0 0.05in; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
      `},getPrintTableCss:d=>{const t=d||{},a=(g,u=40)=>{if(typeof g!="string")return g;let s=g.trim();if(!/^#([0-9a-f]{3}|[0-9a-f]{6})$/i.test(s))return g;s.length===4&&(s="#"+[...s.slice(1)].map(m=>m+m).join(""));const w=parseInt(s.slice(1,3),16),y=parseInt(s.slice(3,5),16),z=parseInt(s.slice(5,7),16),k=.299*w+.587*y+.114*z;if(k<=u)return g;const v=u/k,x=m=>Math.round(m*v).toString(16).padStart(2,"0");return`#${x(w)}${x(y)}${x(z)}`},n=t.header_font_size||13,p=t.header_font_family||"inherit",e=a(t.header_color||"#0f172a"),i=t.header_bg_color||"#f1f5f9",l=t.header_font_weight||"bold",o=t.body_font_size||13,r=t.body_font_family||"inherit",f=t.body_color||"#1e293b",c=t.body_bg_color||"#ffffff",b=t.border_color||"#e2e8f0",h=t.cell_padding!=null?t.cell_padding:6,$=t.section_spacing!=null?t.section_spacing:12;return`
      ...
      `,getPatientHeaderCss:d=>{const t=d||{},a=t.name_size||20,n=t.info_size||13,p=t.barcode_height||35,e=t.qr_size||90,i=t.line_height||1.7;return`
        #Result .patient-header { font-size: ${n}px !important; line-height: ${i} !important; }
        #Result .patient-header .patient-name { font-size: ${a}px !important; }
        #Result .patient-header .info-table td { font-size: ${n}px !important; }
        ...
        .patient-header { font-size: ${n}px !important; line-height: ${i} !important; }
        .patient-header .patient-name { font-size: ${a}px !important; }
        .patient-header .info-table td { font-size: ${n}px !important; }
