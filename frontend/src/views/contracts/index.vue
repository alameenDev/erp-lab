...
const { records, totalCount, record, dialog } = storeToRefs(contractsStore);
const { GetRecords, RemoveContract } = contractsStore;
...
const stats = computed(() => {
  ...
  const withDiscount = list.filter(c => c.discount_percentage > 0).length;
  const totalCreditLimit = list.reduce((sum, c) => sum + (c.credit_limit || 0), 0);
  const avgDiscount = total > 0 ? list.reduce((sum, c) => sum + (c.discount_percentage || 0), 0) / total : 0;
  return { total, withDiscount, totalCreditLimit, avgDiscount };
});
...
const activeFiltersCount = computed(() => {
  ...
  if (filters.value.phone_number) count++;
  if (filters.value.discount_percentage) count++;
  if (filters.value.payment_percent) count++;
  if (filters.value.credit_limit) count++;
  return count;
});
  ...
  return records.value.filter((rec) => {
    const matchesGlobalFilter = globalFilter.value
      ? rec.name?.toLowerCase().includes(globalFilter.value.toLowerCase()) ||
        ...
        rec.phone_number?.toLowerCase().includes(globalFilter.value.toLowerCase()) ||
        rec.discount_percentage?.toString().includes(globalFilter.value) ||
        rec.payment_percent?.toString().includes(globalFilter.value) ||
        rec.credit_limit?.toString().includes(globalFilter.value)
      : true;
    ...
    const matchesPhone = !filters.value.phone_number || rec.phone_number?.toLowerCase().includes(filters.value.phone_number.toLowerCase());
    const matchesDiscount = !filters.value.discount_percentage || rec.discount_percentage?.toString().includes(filters.value.discount_percentage);
    const matchesPayment = !filters.value.payment_percent || rec.payment_percent?.toString().includes(filters.value.payment_percent);
    const matchesCredit = !filters.value.credit_limit || rec.credit_limit?.toString().includes(filters.value.credit_limit);

    return matchesGlobalFilter && matchesName && matchesEmail && matchesPhone && matchesDiscount && matchesPayment && matchesCredit;
  });
});
...
const clearFilters = () => {
  filters.value = { name: "", email: "", phone_number: "", discount_percentage: "", payment_percent: "", credit_limit: "" };
  globalFilter.value = "";
  currentPage.value = 1;
};
...
const exportToExcel = () => {
  const exportData = filteredRecords.value.map((rec, i) => ({
    ...
    [t("discount_percentage")]: rec.discount_percentage,
    [t("payment_percent")]: rec.payment_percent,
    [t("credit_limit")]: rec.credit_limit,
  }));
...
const addRecord = () => {
  ...
  record.value.payment_percent = 0;
  record.value.maximum_payment_per_invoice = 0;
  record.value.credit_limit = 0;
  record.value.price_limit = 0;
  ...
  <div class="space-y-6">
            ...
            <div class="flex items-center gap-3 mb-3">
              ...
              <div>
                <span class="px-3 py-1 bg-primary-500/20 text-primary-300 text-xs font-semibold rounded-full">
                  {{ stats.total }} {{ t("contracts") }}
                </span>
              </div>
            </div>
            <h1 class="text-2xl lg:text-3xl font-bold text-white mb-2">{{ t("contract_Management") }}</h1>
            <p class="text-slate-400 text-sm lg:text-base">{{ t("manage_contracts") || "Manage contracts and agreements" }}</p>
          </div>
          ...
          <div class="flex gap-3 flex-wrap">
            <div class="bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/10 min-w-[120px]">
              <div class="flex items-center gap-2 mb-2">
                ...
                <span class="text-xs text-slate-400">{{ t("with_discount") || "With Discount" }}</span>
              </div>
            ...
            <div class="bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/10 min-w-[120px]">
              <div class="flex items-center gap-2 mb-2">
                ...
                <span class="text-xs text-slate-400">{{ t("avg_discount") || "Avg Discount" }}</span>
              </div>
              <p class="text-2xl font-bold text-white">{{ stats.avgDiscount.toFixed(1) }}%</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-slate-500 mb-1">{{ t("total") }} {{ t("contracts") }}</p>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-slate-500 mb-1">{{ t("with_discount") || "With Discount" }}</p>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-slate-500 mb-1">{{ t("total_credit_limit") || "Total Credit Limit" }}</p>
            <p class="text-2xl font-bold text-blue-600">{{ formatCurrency(stats.totalCreditLimit) }}</p>
          </div>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-slate-500 mb-1">{{ t("avg_discount") || "Avg Discount" }}</p>
            <p class="text-2xl font-bold text-amber-600">{{ stats.avgDiscount.toFixed(1) }}%</p>
          </div>
        ...
        <div v-if="showFilters" class="overflow-hidden">
          <div class="px-5 pb-5 pt-2 border-t border-slate-100 bg-slate-50/50">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
              ...
              <!-- Discount Filter -->
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">{{ t("discount_percentage") }}</label>
                <input
              ...
              <!-- Payment Percent Filter -->
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">{{ t("payment_percent") }}</label>
                <input
                  v-model="filters.payment_percent"
                  type="text"
                  :placeholder="t('payment_percent') + '...'"
                  class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-slate-700 focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all"
                />
              </div>
              <!-- Credit Limit Filter -->
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">{{ t("credit_limit") }}</label>
                <input
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden">
      ...
      <div v-else-if="filteredRecords.length === 0" class="p-12 text-center">
        ...
        <p class="text-slate-500 mb-6 max-w-md mx-auto">{{ t("no_contracts_message") || "No contracts found. Add your first contract to get started." }}</p>
        <button
          v-if="havePermission('contracts create')"
          @click="addRecord"
          ...
          <table class="w-full">
            <thead>
              <tr class="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200">
                ...
                <th class="px-5 py-4 text-start text-xs font-semibold text-slate-600 uppercase tracking-wider">{{ t("payment_percent") }}</th>
                <th class="px-5 py-4 text-start text-xs font-semibold text-slate-600 uppercase tracking-wider">{{ t("credit_limit") }}</th>
                <th class="px-5 py-4 text-start text-xs font-semibold text-slate-600 uppercase tracking-wider">{{ t("actions") }}</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
              <tr
                v-for="(contract, index) in paginatedRecords"
                :key="contract.id"
              ...
              >
                ...
                <td class="px-5 py-4">
                  <span class="text-sm font-medium text-slate-800">{{ contract.name }}</span>
                </td>
                <td class="px-5 py-4">
                  <span class="text-sm text-slate-600">{{ contract.email || "----" }}</span>
                </td>
                <td class="px-5 py-4" dir="ltr">
                  <span class="text-sm text-slate-600">{{ contract.phone_number || "----" }}</span>
                </td>
                <td class="px-5 py-4">
                  <span v-if="contract.discount_percentage" class="inline-flex items-center px-2.5 py-1 bg-green-50 text-green-700 text-xs font-medium rounded-lg">
                    {{ contract.discount_percentage }}%
                  </span>
                  <span v-else class="text-slate-400">-</span>
                </td>
                <td class="px-5 py-4">
                  <span v-if="contract.payment_percent" class="inline-flex items-center px-2.5 py-1 bg-blue-50 text-blue-700 text-xs font-medium rounded-lg">
                    {{ contract.payment_percent }}%
                  </span>
                  <span v-else class="text-slate-400">-</span>
                </td>
                <td class="px-5 py-4">
                  <span v-if="contract.credit_limit" class="inline-flex items-center px-2.5 py-1 bg-amber-50 text-amber-700 text-xs font-medium rounded-lg">
                    {{ formatCurrency(contract.credit_limit) }}
                  </span>
                  <span v-else class="text-slate-400">-</span>
                </td>
                <td class="px-5 py-4">
