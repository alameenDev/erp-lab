...
const { records, totalCount, record, dialog } = storeToRefs(referralsStore);
const { GetRecords, RemoveReferral } = referralsStore;
...
const stats = computed(() => {
  ...
  const doctors = list.filter(r => r.role?.toLowerCase() === 'doctor').length;
  const labs = list.filter(r => r.role?.toLowerCase() === 'lab').length;
  const totalCommission = list.reduce((sum, r) => sum + (r.commission || 0), 0);
  return { total, doctors, labs, totalCommission };
});
...
const activeFiltersCount = computed(() => {
  ...
  if (filters.value.commission) count++;
  return count;
});
  ...
  return records.value.filter((rec) => {
    const matchesGlobalFilter = globalFilter.value
      ? rec.name?.toLowerCase().includes(globalFilter.value.toLowerCase()) ||
        ...
        rec.commission?.toString().includes(globalFilter.value) ||
    ...
    const matchesLab = !filters.value.lab || rec.lab?.toLowerCase().includes(filters.value.lab.toLowerCase());
    ...
    const matchesRole = !filters.value.role || rec.role?.toLowerCase() === filters.value.role.toLowerCase();
    const matchesCommission = !filters.value.commission || rec.commission?.toString().includes(filters.value.commission);

    return matchesGlobalFilter && matchesName && matchesLab && matchesEmail && matchesPhone && matchesRole && matchesCommission;
  });
});
...
const clearFilters = () => {
  filters.value = { name: "", email: "", role: "", phone_number: "", lab: "", commission: "" };
  globalFilter.value = "";
  currentPage.value = 1;
};
...
const exportToExcel = () => {
  const exportData = filteredRecords.value.map((rec, index) => ({
    ...
    [t("commission")]: rec.commission,
    [t("userRole")]: rec.role,
  }));
  const ws = XLSX.utils.json_to_sheet(exportData);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "Referrals");
  XLSX.writeFile(wb, "referrals_export.xlsx");
};

const addRecord = () => {
  Object.keys(record.value).forEach((key) => { record.value[key] = null; });
  record.value.id = null;
  record.value.commission = 0;
  dialog.value = true;
};

const editRecord = (rec) => {
  Object.assign(record.value, rec);
  record.value.role_id = UserRoles?.asList().find((v) => v.label.toLowerCase() === rec.role?.toLowerCase())?.value || null;
  dialog.value = true;
};
  ...
  <div class="space-y-6">
            ...
            <div class="flex items-center gap-3 mb-3">
              ...
              <div>
                <span class="px-3 py-1 bg-primary-500/20 text-primary-300 text-xs font-semibold rounded-full">
                  {{ stats.total }} {{ t("referrals") }}
                </span>
              </div>
            </div>
            <h1 class="text-2xl lg:text-3xl font-bold text-white mb-2">{{ t("referralsManagment") }}</h1>
            <p class="text-slate-400 text-sm lg:text-base">{{ t("manage_referrals") || "Manage referrals and commissions" }}</p>
          </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-slate-500 mb-1">{{ t("total") }} {{ t("referrals") }}</p>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-slate-500 mb-1">{{ t("total_commission") || "Total Commission" }}</p>
            <p class="text-2xl font-bold text-green-600">{{ formatCurrency(stats.totalCommission) }}</p>
          </div>
        ...
        <div v-if="showFilters" class="overflow-hidden">
          <div class="px-5 pb-5 pt-2 border-t border-slate-100 bg-slate-50/50">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
              ...
              <!-- Commission Filter -->
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">{{ t("commission") }}</label>
                <input
              ...
              <div v-if="UserRoles">
                ...
                >
                  ...
                  <option v-for="role in UserRoles.asList()" :key="role.value" :value="role.label">{{ role.label }}</option>
                </select>
              </div>
            </div>
          </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden">
      ...
      <div v-else-if="filteredRecords.length === 0" class="p-12 text-center">
        ...
        <p class="text-slate-500 mb-6 max-w-md mx-auto">{{ t("no_referrals_message") || "No referrals found. Add your first referral to get started." }}</p>
        <button
          v-if="havePermission('referrals create')"
          @click="addRecord"
          ...
          <table class="w-full">
            <thead>
              <tr class="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200">
                ...
                <th class="px-5 py-4 text-start text-xs font-semibold text-slate-600 uppercase tracking-wider">{{ t("commission") }}</th>
            ...
            <tbody class="divide-y divide-slate-100">
              <tr
                v-for="(referral, index) in paginatedRecords"
                :key="referral.id"
              ...
              >
                ...
                <td class="px-5 py-4">
                  <span class="text-sm font-medium text-slate-800">{{ referral.name }}</span>
                </td>
                <td class="px-5 py-4">
                  <span class="text-sm text-slate-600">{{ referral.lab || "----" }}</span>
                </td>
                <td class="px-5 py-4">
                  <span class="text-sm text-slate-600">{{ referral.email || "----" }}</span>
                </td>
                <td class="px-5 py-4" dir="ltr">
                  <span class="text-sm text-slate-600">{{ referral.phone_number || "----" }}</span>
                </td>
                <td class="px-5 py-4">
                  <span v-if="referral.commission" class="inline-flex items-center px-2.5 py-1 bg-green-50 text-green-700 text-xs font-medium rounded-lg">
                ...
                <td class="px-5 py-4">
                  <span
                    ...
                    :class="getRoleBadgeClass(referral.role)"
                  >
                    <span class="w-2 h-2 rounded-full" :class="getRoleDotClass(referral.role)"></span>
                    {{ referral.role || "----" }}
                  </span>
                </td>
                <td class="px-5 py-4">
