...
const { testdialog, tests } = storeToRefs(reportsStore);
...
const totalPages = computed(() => {
  return Math.ceil((tests.value?.length || 0) / rowsPerPage.value);
});
      ...
      <div v-if="testdialog" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          <div class="flex items-center justify-between p-4 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">{{ t("tests") }}</h2>
          ...
          <div class="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
            <div v-if="tests?.length" class="overflow-x-auto">
              <table class="w-full">
                <thead>
                  <tr class="bg-gray-50 border-b border-gray-200">
                    <th class="px-4 py-3 text-center text-sm font-semibold text-gray-700">{{ t("name") }}</th>
                    <th class="px-4 py-3 text-center text-sm font-semibold text-gray-700">{{ t("Original_Price") }}</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-gray-200">
                  <tr v-for="test in paginatedTests" :key="test.id" class="hover:bg-gray-50">
                    <td class="px-4 py-3 text-center text-gray-700">{{ test.name }}</td>
                    <td class="px-4 py-3 text-center text-gray-700">{{ test.price }}</td>
                  </tr>
                </tbody>
              </table>
