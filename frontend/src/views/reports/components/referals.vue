...
const { referalsdialog, referals } = storeToRefs(reportsStore);
...
const paginatedReferals = computed(() => {
  const start = (currentPage.value - 1) * rowsPerPage.value;
  const end = start + rowsPerPage.value;
  return referals.value?.slice(start, end) || [];
});

const totalPages = computed(() => {
  return Math.ceil((referals.value?.length || 0) / rowsPerPage.value);
});

const close = () => {
  referals.value = [];
  referalsdialog.value = false;
};
</script>
...
<template>
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="referalsdialog" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          <div class="flex items-center justify-between p-4 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">{{ t("referals") }}</h2>
          ...
          <div class="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
            <div v-if="referals?.length" class="overflow-x-auto">
              <table class="w-full">
                <thead>
                ...
                <tbody class="divide-y divide-gray-200">
                  <tr v-for="referal in paginatedReferals" :key="referal.id" class="hover:bg-gray-50">
                    <td class="px-4 py-3 text-center text-gray-700">{{ referal.name }}</td>
                    <td class="px-4 py-3 text-center text-gray-700">{{ referal.count }}</td>
                    <td class="px-4 py-3 text-center text-gray-700">{{ referal.discount_percentage }}</td>
                  </tr>
                </tbody>
              </table>
