...
const { culturedialog, cultures } = storeToRefs(reportsStore);
...
const paginatedCultures = computed(() => {
  ...
  return cultures.value?.slice(start, end) || [];
});

const totalPages = computed(() => {
  return Math.ceil((cultures.value?.length || 0) / rowsPerPage.value);
});
      ...
      <div v-if="culturedialog" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          <div class="flex items-center justify-between p-4 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">{{ t("cultures") }}</h2>
          ...
          <div class="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
            <div v-if="cultures?.length" class="overflow-x-auto">
              <table class="w-full">
                <thead>
                ...
                <tbody class="divide-y divide-gray-200">
                  <tr v-for="culture in paginatedCultures" :key="culture.id" class="hover:bg-gray-50">
                    <td class="px-4 py-3 text-center text-gray-700">{{ culture.name }}</td>
                    <td class="px-4 py-3 text-center text-gray-700">{{ culture.count }}</td>
                    <td class="px-4 py-3 text-center text-gray-700">{{ culture.total }}</td>
                  </tr>
                </tbody>
              </table>
