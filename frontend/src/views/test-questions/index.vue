...
const { testsQuestions, totalCount, record, dialog } = storeToRefs(testsQuestionsStore);
const { GetTestsQuestions, RemoveTestsQuestions } = testsQuestionsStore;
const { GetanswerTypes } = answerTypesStore;
...
const stats = computed(() => {
  const list = testsQuestions.value || [];
  const uniqueAnswerTypes = new Set(list.map(q => q.answer_type).filter(Boolean));
  const uniqueLabs = new Set(list.map(q => q.lab).filter(Boolean));

  return {
    total: totalCount.value || 0,
    answerTypes: uniqueAnswerTypes.size,
...
const activeFiltersCount = computed(() => {
  let count = 0;
  if (filters.value.question) count++;
  if (filters.value.answer_type) count++;
...
const filteredRecords = computed(() => {
  if (!testsQuestions.value) return [];
  return testsQuestions.value.filter((rec) => {
    return (
      (!filters.value.question || rec.question?.toLowerCase().includes(filters.value.question.toLowerCase())) &&
      (!filters.value.answer_type || rec.answer_type?.toLowerCase().includes(filters.value.answer_type.toLowerCase())) &&
...
// Methods
const clearFilters = () => {
  filters.value = { question: "", answer_type: "", lab: "" };
  currentPage.value = 1;
};
...
const exportToExcel = () => {
  const exportData = filteredRecords.value.map((rec, index) => ({
    "#": index + 1,
    [t("question")]: rec.question,
    [t("answer_type")]: rec.answer_type,
    [t("lab/bruanch")]: rec.lab,
  }));
  const ws = XLSX.utils.json_to_sheet(exportData);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "TestQuestions");
  XLSX.writeFile(wb, "test_questions_export.xlsx");
};
...
const editRecord = (rec) => {
  ...
  record.value.answer_type_id_fk = rec.answer_type_id;
  dialog.value = true;
};
...
const downloadTemplate = () => {
  ...
  window.open(`${baseUrl}/tests-questions/download-template`, "_blank");
};
...
const submitImport = async () => {
  ...
  importResult.value = null;
  try {
    const data = await testsQuestionsStore.ImportTestsQuestions(importFile.value);
  ...
  <div class="space-y-6">
            ...
            <div class="flex items-center gap-3 mb-3">
              ...
              <div>
                <span class="px-3 py-1 bg-primary-500/20 text-primary-300 text-xs font-semibold rounded-full">
                  {{ stats.total }} {{ t("test-questions") }}
                </span>
              </div>
            </div>
            <h1 class="text-2xl lg:text-3xl font-bold text-white mb-2">{{ t("test-questions") }}</h1>
            <p class="text-slate-400 text-sm lg:text-base">{{ t("manage_test_questions") }}</p>
          </div>
          ...
          <div class="flex gap-3 flex-wrap">
            ...
            <div class="bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/10 min-w-[120px]">
              <div class="flex items-center gap-2 mb-2">
                ...
                <span class="text-xs text-slate-400">{{ t("answer_types") }}</span>
              </div>
              <p class="text-2xl font-bold text-white">{{ stats.answerTypes }}</p>
            </div>
            <div class="bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/10 min-w-[120px]">
              <div class="flex items-center gap-2 mb-2">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-slate-500 mb-1">{{ t("total") }} {{ t("questions") }}</p>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-slate-500 mb-1">{{ t("answer_types") }}</p>
            <p class="text-2xl font-bold text-slate-800">{{ stats.answerTypes }}</p>
          </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden">
        ...
        <div v-if="showFilters" class="overflow-hidden">
          <div class="px-5 pb-5 pt-2 border-t border-slate-100 bg-slate-50/50">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <!-- Answer Type Filter -->
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">{{ t("answer_type") }}</label>
                <input
      ...
      <div v-else-if="totalCount === 0" class="p-12 text-center">
        ...
        <p class="text-slate-500 mb-6 max-w-md mx-auto">{{ t("no_questions_desc") }}</p>
        <button
          v-if="havePermission('test questions create')"
          @click="addRecord"
          ...
          <table class="w-full">
            <thead>
              <tr class="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200">
                ...
                <th class="px-5 py-4 text-start text-xs font-semibold text-slate-600 uppercase tracking-wider">{{ t("question") }}</th>
                <th class="px-5 py-4 text-start text-xs font-semibold text-slate-600 uppercase tracking-wider">{{ t("answer_type") }}</th>
            ...
            <tbody class="divide-y divide-slate-100">
              <tr
                v-for="(question, index) in paginatedRecords"
                :key="question.id"
              ...
              >
                ...
                <td class="px-5 py-4">
                  <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0">
                      <p class="font-semibold text-slate-800 truncate">{{ question.question }}</p>
                    </div>
                  </div>
                </td>
                ...
                <!-- Answer Type -->
                <td class="px-5 py-4">
                  <span v-if="question.answer_type" class="inline-flex items-center px-3 py-1.5 bg-purple-50 text-purple-700 text-xs font-medium rounded-lg">
                ...
                <!-- Lab -->
                <td class="px-5 py-4">
                  <span v-if="question.lab" class="inline-flex items-center px-2.5 py-1 bg-blue-50 text-blue-700 text-xs font-medium rounded-lg">
                    {{ question.lab }}
                  </span>
                  <span v-else class="text-slate-400">-</span>
                </td>
    ...
    <!-- Import Modal -->
    <UiModal v-model="importDialog" :title="t('import') + ' ' + t('test-questions')" max-width="lg">
      <div class="space-y-5">
