...
const { Testcategories, totalCount, record, dialog } = storeToRefs(categoriesStore);
const { Getcategories, Removecategory } = categoriesStore;
...
const downloadTemplate = () => {
  ...
  link.href = "/api/categories/download-template";
  link.download = "categories_import_template.xlsx";
...
const submitImport = async () => {
  ...
  importResult.value = null;
  try {
    const data = await categoriesStore.ImportCategories(importFile.value);
    importResult.value = data;
    if (data.created > 0) {
      toast.success(`${data.created} ${t("categories")} ${t("alertSuccess")}`);
    }
  } catch (error) {
...
const stats = computed(() => {
  const categories = Testcategories.value || [];
  const uniqueLabs = new Set(categories.map(c => c.lab).filter(Boolean));

  return {
    totalCategories: totalCount.value || 0,
...
const filteredRecords = computed(() => {
  if (!Testcategories.value) return [];
  return Testcategories.value.filter((rec) => {
    return (
  ...
  <div class="space-y-6">
            ...
            <div class="flex items-center gap-3 mb-3">
              ...
              <div>
                <span class="px-3 py-1 bg-primary-500/20 text-primary-300 text-xs font-semibold rounded-full">
                  {{ stats.totalCategories }} {{ t("categories") }}
                </span>
              </div>
            </div>
            <h1 class="text-2xl lg:text-3xl font-bold text-white mb-2">{{ t("categories") }}</h1>
            <p class="text-slate-400 text-sm lg:text-base">{{ t("manage_categories") }}</p>
          </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-slate-500 mb-1">{{ t("total") }} {{ t("categories") }}</p>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden">
      ...
      <div v-else-if="totalCount === 0" class="p-12 text-center">
        ...
        <p class="text-slate-500 mb-6 max-w-md mx-auto">{{ t("no_categories_message") }}</p>
        <button
          v-if="havePermission('categories create')"
          @click="addRecord"
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody class="divide-y divide-slate-100">
              <tr
                v-for="(category, index) in paginatedRecords"
                :key="category.id"
              ...
              >
                ...
                <td class="px-5 py-4">
                  <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center flex-shrink-0 shadow-lg shadow-primary-500/20">
                      <span class="text-white font-bold text-sm">{{ category.name?.charAt(0)?.toUpperCase() || 'C' }}</span>
                    </div>
                    <div class="min-w-0">
                ...
                <!-- Lab/Branch -->
                <td class="px-5 py-4">
                  <span v-if="category.lab" class="inline-flex items-center px-2.5 py-1 bg-blue-50 text-blue-700 text-xs font-medium rounded-lg">
                    {{ category.lab }}
                  </span>
