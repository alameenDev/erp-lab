...
<script>
     ...
     export default {
          ...
          computed: {
               ...
               ...mapWritableState(usesamplesStore, ["samplesList", "totalCount", "record", "dialog"]),
               filteredRecords() {
                    return this.samplesList.filter((record) => {
                         const matchesGlobalFilter = this.globalFilter
                              ? record.sample_name?.toLowerCase().includes(this.globalFilter.toLowerCase()) ||
                                record.lab?.toLowerCase().includes(this.globalFilter.toLowerCase())
                              : true;

                         const matchesColumnFilters =
                              !this.Filters.name ||
                              (record.sample_name.toLowerCase().includes(this.Filters.name.toLowerCase()) &&
                                   !this.Filters.lab) ||
          ...
          methods: {
               ...mapActions(usesamplesStore, ["Getsamples", "Removesample"]),
               deleteRecord(record) {
               ...
               editRecord(record) {
                    ...
                    this.record.name = record.sample_name;
                    this.dialog = true;
               },
               addRecord() {
